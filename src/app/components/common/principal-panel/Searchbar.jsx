...
import { useUserProfileQuery } from "@/redux/features/principal-panel/profile/profileApi";
...
const Searchbar = () => {
  ...
  useEffect(() => {
    if (
      data &&
      data.user &&
      data.user.principal_profile &&
      data.user.principal_profile.school
    ) {
      const schoolId = data.user.principal_profile.school.id;
