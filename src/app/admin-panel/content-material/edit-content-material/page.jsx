...
import { editContent, uploadContent } from "@/services/admin/contentupload";
import { toast } from "react-toastify";
import { useSelector } from "react-redux";
import { useSearchParams } from "next/navigation";
import { fetchContentMaterialDetail } from "@/services/admin/powerclass";
...
const MyComponent = () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        let data = {
          ...
          class_grade: content.class,
          title: content.title,
        };
        setuploadmedia(content.upload_media);
        setFormData(data);
      } else {
      }
    } catch (e) {
  ...
  const deletecontent = (id, index) => {
    setisLoading(true);
    axios
      .delete(`${base_url}/admin/content-upload/media/${id}`, {
        headers: {
      ...
      .then((res) => {
        if (res.data.success) {
          ...
          toast.success("Uploaded Media deleted successfully");
          data.splice(index, 1);
          setuploadmedia(data);
        } else {
    ...
    <>
      <div className="content_upload_div">
        <div className="content_upload_heading mb-4 d-flex justify-content-between align-items-center">
          <h2 className="medium_font font_size_24">Edit Uploaded Content </h2>
        </div>
          ...
          <div className="content_upload_card_div card_border rounded-4 p-xxl-4 p-xl-4 p-lg-4 p-md-4">
            ...
            <div className="uploaded_file_div mt-3">
              ...
              {uploadmedia.map((item, index) => (
                <div
                  key={index}
                  className="mb-2 uploaded_file_card border border-success rounded p-2 d-flex justify-content-between align-itmes-center"
                >
                  <div className="text_div position-relative">
                    <p className="font_size_14 mb-0">
                      {" "}
                      Uploaded Content - {index + 1}{" "}
                    </p>
                    <div className="border_div"></div>
                  </div>
                  <div className="uploaded_file_icon_div">
            ...
            <div className="btn_div my-5" style={{ width: "250px" }}>
              <Button text="Edit Content" type={"submit"} className="py-3" />
            </div>
          </div>
        </form>
      </div>
